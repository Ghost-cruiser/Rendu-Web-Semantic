    ...
    try {  
        ...
        if ($sql && count($sql)) {  
            ...
            $sql->bindValue(":nom", $nom ? $nom : null);   
            ...
            $sql->bindValue(":prenom", $prenom ? $prenom : null);   
            $sql->bindValue(":tel", $tel ? $tel : null);   
            $sql->bindValue(":website", $website ? $website : null);   
            $sql->bindValue(":birthdate", $birthdate ? $birthdate : null);   
            $sql->bindValue(":ville", $ville ? $ville : null);   
            $sql->bindValue(":taille", $taille ? $taille : null);   
            $sql->bindValue(":profilepic", $profilepic ? $profilepic : null);   
            // idem pour la couleur, attention au format ici (7 caractères, 6 caractères attendus seulement)  
            $sql->bindValue(":couleur", $couleur ? substr($couleur, 1) : null);   

            // idem pour le sexe, attention il faut être sûr que c'est bien 'H', 'F', ou ''  
            $sql->bindValue(":sexe", $sexe && ($sexe == "H" || $sexe == "F") ? : "");   
            ...
            if (!$sql->execute()) {  
                ...
                if ($sql->rowCount()<1) {  
                    ...
                    $_SESSION["userpic"] = $user["profilepic"];
                    $_SESSION["usercolor"] = $user["couleur"];
